...
# PROBLEMA identificado no gráfico de resíduos: a distribuição do Teste tem
# cauda assimétrica à direita (erros chegando a +40), indicando que o modelo
# subestima amostras com CBR muito alto (solos resistentes, raros no dataset).
#
# CAUSA: a distribuição do CBR é positivamente assimétrica (skewed right) —
# a maioria das amostras tem CBR baixo/médio, e poucos casos têm CBR > 50%.
# Redes neurais treinadas com MAE/MSE tendem a "puxar" as previsões para a
# média, ignorando as caudas.
#
# SOLUÇÃO: treinar na escala logarítmica.
# - log1p(CBR) comprime a cauda direita, tornando a distribuição mais simétrica
# - O modelo passa a tratar erros relativos igualmente (um erro de 10 em CBR=5
#   é tão grave quanto um erro de 20 em CBR=10)
# - Após a previsão, aplica-se expm1() para voltar à escala original
#
# Como ativar: defina LOG_ALVO = True
# Como desativar: LOG_ALVO = False (comportamento anterior)
LOG_ALVO = True                  # True = treina em log1p(CBR), prevê em expm1(pred)

# ── Pesos amostrais ───────────────────────────────────────────────────────────
# PROBLEMA: o gráfico mostra que amostras com CBR alto (caudas > +10 no teste)
# são consistentemente subestimadas — o modelo não "vê" exemplos suficientes
# nessa faixa para aprender bem.
#
# SOLUÇÃO: dar peso extra às amostras com CBR > THRESHOLD durante o treino,
# forçando o modelo a reduzir o erro nessa região menos representada.
# W_MINOR=3.0 significa que cada amostra com CBR alto "conta 3 vezes" no
# cálculo do gradiente.
USE_WEIGHTS = True               # True = amostras com CBR alto recebem mais atenção
THRESHOLD   = 20.0               # Solos com CBR > 20% são considerados "raros e importantes"
W_MINOR     = 3.0                # Peso das amostras acima do threshold
W_MAJOR     = 1.0                # Peso das amostras abaixo do threshold

# ── Busca de hiperparâmetros ──────────────────────────────────────────────────
# Espaço de busca inspirado no MLPBlockSpace (AG):
...
# Aumentado de 20 para 50: cobre melhor o espaço de hiperparâmetros
# (com log-transform e pesos, o modelo se comporta diferente — mais busca = melhor ajuste)
N_ITER_BUSCA   = 50
...
def exibir_metricas(y_true: np.ndarray, y_pred: np.ndarray, nome: str) -> dict:
    """
    Calcula e imprime MSE, RMSE, MAE, MAPE e R² para um conjunto de previsões.
    Alerta se MSE <= 1, o que indica possível problema de escala ou data leakage.

    A dissertação (Moreira, 2025) exige as 4 métricas: R², RMSE, MAE e MAPE.
    MAPE = média dos erros percentuais absolutos — facilita interpretação relativa.
    ...
    rmse = np.sqrt(mse)                          # Raiz do MSE: mesma unidade do alvo (%)
    mae  = mean_absolute_error(y_true, y_pred)  # Média dos erros absolutos — perda primária
    r2   = r2_score(y_true, y_pred)             # R²: 1.0 = perfeito, 0.0 = modelo constante

    # MAPE: erro percentual médio — evita divisão por zero com clip(1e-6)
    # Interpretação: "em média, o modelo erra X% do valor real do CBR"
    # Exigido explicitamente pela dissertação como 4ª métrica de avaliação
    mape = np.mean(np.abs((y_true - y_pred) / np.clip(np.abs(y_true), 1e-6, None))) * 100

    print(f"\n  [{nome}]")
    print(f"    MSE  : {mse:.4f}")

    # MSE <= 1 é suspeito para CBR (faixa ~2% a ~120%):
    # pode indicar alvo normalizado acidentalmente ou data leakage
    if mse <= 1:
        print(f"    ATENCAO: MSE={mse:.4f} <= 1 — verifique escala ou data leakage.")

    print(f"    RMSE : {rmse:.4f}")
    print(f"    MAE  : {mae:.4f}")               # Perda otimizada diretamente durante o treino
    print(f"    MAPE : {mape:.2f}%")             # Erro percentual — comparável com literatura
    print(f"    R²   : {r2:.4f}")               # Fitness principal: variância explicada
...
# Transforma o alvo para escala logarítmica (se ativado)
# log1p(x) = log(1+x): estável mesmo para CBR próximos de zero
# Armazena Y_orig para calcular as métricas finais na escala real do CBR (%)
Y_orig = Y.copy()
if LOG_ALVO:
    Y = np.log1p(Y)
    print(f"     Log-transform ativado: alvo transformado para log1p(CBR)")
    print(f"     Faixa original: [{Y_orig.min():.2f}, {Y_orig.max():.2f}]  →  log1p: [{Y.min():.4f}, {Y.max():.4f}]")
...
# CORREÇÃO: validation_data usa X_val_n/Y_val (não o teste) para o EarlyStopping
# decidir quando parar — evita que o modelo seja otimizado para o conjunto de teste
historico = modelo_final.fit(
    X_tv_n, Y_tv,
    epochs=500,                                # Limite maior — EarlyStopping decide quando parar
    batch_size=melhor_config["batch_size"],    # Batch size otimizado pela busca
    validation_data=(X_val_n, Y_val),          # Monitoramento na validação (não no teste)
    sample_weight=sw_tv,
...
# Se o alvo foi transformado em log, reverte as previsões e os rótulos para a
# escala original do CBR antes de calcular as métricas — garante interpretabilidade
if LOG_ALVO:
    pred_val   = np.expm1(pred_val)    # expm1(x) = exp(x)-1: inverso exato de log1p
    ...
    print("     Previsoes revertidas para escala original (expm1)")
else:
    Y_val_met   = Y_val
    y_teste_met = y_teste

# Calcula e imprime todas as métricas (MSE, RMSE, MAE, MAPE, R²)
met_val   = exibir_metricas(Y_val_met,   pred_val,   "Validacao")
...
lim = [min(Y_val_met.min(), pred_val.min()) * 0.95,       # Limite inferior com margem de 5%
       max(Y_val_met.max(), pred_val.max()) * 1.05]        # Limite superior com margem de 5%
...
ax1.set_xlabel("Real CBR (%)"); ax1.set_ylabel("Previsto CBR (%)")
...
ax2.scatter(y_teste_met, pred_teste, alpha=0.6, color=PALETTE["verde"],
            edgecolors="white", linewidths=0.4, s=50)
...
ax2.set_xlabel("Real CBR (%)"); ax2.set_ylabel("Previsto CBR (%)")
...
table_data = [
    ...
    ["MAPE",  f"{met_val['mape']:.2f}%", f"{met_teste['mape']:.2f}%"],  # Exigido pela dissertação
...
ax3.set_title("Comparativo de Metricas (escala original)", fontweight="bold", pad=14)

# D) Resíduos — Validação
# Resíduo = Real - Previsto na escala original (%); distribuição em torno de 0 = sem viés
ax4 = fig.add_subplot(gs[1, 0])
residuos_val = Y_val_met - pred_val            # Resíduos na escala original do CBR
...
ax4.set_xlabel("Previsto CBR (%)"); ax4.set_ylabel("Residuo (Real - Previsto)")

# E) Resíduos — Teste
# Após log-transform os resíduos devem ficar mais simétricos (sem cauda direita longa)
ax5 = fig.add_subplot(gs[1, 1])
residuos_teste = y_teste_met - pred_teste      # Resíduos na escala original do CBR
...
ax5.set_xlabel("Previsto CBR (%)"); ax5.set_ylabel("Residuo (Real - Previsto)")
...
# Após log-transform a assimetria direita do Teste deve ser reduzida
fig2, axes2 = plt.subplots(1, 2, figsize=(14, 5), facecolor=PALETTE["fundo"])
fig2.suptitle("Distribuicao dos Residuos (escala original CBR %)", fontsize=14, fontweight="bold")
