...
OUTPUT_DIR = os.path.normpath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "models", "mlp"
))
...
N_ITER_BUSCA   = 30               # Aumentado de 20 para 30 — melhor cobertura
...
def grafico_data_leakage(hist):
    ...
    fig.suptitle("Verificação de Data Leakage", fontsize=14, fontweight="bold")
    ...
    ax.set_title("MAE Treino vs Validação", fontweight="bold")
    ...
    ax2.set_title("Gap val − treino por Época", fontweight="bold")
...
def grafico_outliers(residuos_val, residuos_teste):
    ...
    """
    fig, axes = plt.subplots(1, 2, figsize=(14, 5), facecolor=PALETTE["fundo"])
    fig.suptitle("Outliers nos Resíduos (1.5 × IQR)", fontsize=14, fontweight="bold")
    ...
    for ax, res, titulo, cor, cor_norm in zip(
        ...
        n_out = len(outliers)
        ax.set_title(f"{titulo} — {n_out} outlier(s) detectado(s)", fontweight="bold")
...
def grafico_early_stopping(hist, epocas_reais, patience):
    ...
    ax.set_title(f"Early Stopping — patience={patience} épocas", fontweight="bold")
    ax.set_xlabel("Época"); ax.set_ylabel("MAE")
...
def grafico_r2_comparativo(r2_treino, r2_val, r2_teste):
    ...
    ax.set_title("R² por Conjunto", fontweight="bold")
    ax.set_xlabel("R²")
    ax.legend(framealpha=0)
    plt.tight_layout()
    plt.show()
...
def grafico_raciocinio_rede(hist, epocas_reais):
    ...
    fig.suptitle("Raciocínio da Rede Neural — Evolução do Aprendizado",
    ...
    ax1.set_ylabel("MAE"); ax1.set_title("Curva de Aprendizado com Marcos de Mudança",
                                          fontweight="bold")
    ...
    ax2.set_title("Derivada do Val MAE — Onde o Raciocínio Mudou", fontweight="bold")
...
for i, cfg in enumerate(combinacoes):
    ...
    if mae_val < melhor_mae_val:
        ...
        melhor_config  = cfg
    print(f"     [{i+1:2d}/{N_ITER_BUSCA}] lr={cfg['lr']:.4f}  "
          f"batch={cfg['batch_size']}  drop={cfg['dropout']:.2f}  "
...
fig.suptitle("MLP — Análise de Performance", fontsize=16, fontweight="bold", y=0.98)
...
ax1.set_title("Previsto vs Real — Validação", fontweight="bold")
...
ax3.set_title("Comparativo de Métricas", fontweight="bold", pad=14)
...
ax4.set_title("Resíduos — Validação", fontweight="bold")
...
ax5.set_title("Resíduos — Teste", fontweight="bold")
...
ax6.set_title("Histórico de Loss (MAE)", fontweight="bold")
...
fig2.suptitle("Distribuição dos Resíduos", fontsize=14, fontweight="bold")
...
if "mse" in historico.history:
    ...
    ax7.set_title("Histórico de MSE", fontweight="bold")
...
ax8.set_title("Busca de Hiperparâmetros — MAE de Validação por Combinação", fontweight="bold")
