...
DATA_PATH = os.path.normpath(os.path.join(CODE_DIR, "..", "data", "processed", "dados_processados_1.csv"))
RF_PT_DIR = os.path.normpath(os.path.join(CODE_DIR, "..", "models", "rf"))
RF_EN_DIR = os.path.normpath(os.path.join(CODE_DIR, "..", "models", "rf_en"))
MLP_DIR   = os.path.normpath(os.path.join(CODE_DIR, "..", "models", "mlp"))
QNT_DIR   = os.path.normpath(os.path.join(CODE_DIR, "..", "models", "rf_quintis"))
    ...
    *Dissertação de Mestrado · PUC Goiás · Engenharia Industrial e IA · 2025*
    ...
    try:
        ...
        with col_hist:
            ...
            ax.set_title("Distribuição do CBR", fontweight="bold")
            ax.legend(framealpha=0)
            plt.tight_layout()
        ...
        ax2.set_title("Correlação de Pearson (azul = positiva · vermelho = negativa)",
...
# ─────────────────────────────────────────────
    ...
    with aba_pt:
        met_pt = pagina_rf(RF_PT_DIR, "rf_modelo_final.joblib", "Português")
    with aba_en:
        met_en = pagina_rf(RF_EN_DIR, "rf_model_final.joblib", "English")
    with aba_comp:
        ...
        if met_pt is None or met_en is None:
            ...
            ax.set_title("Comparação de Métricas — Teste Final", fontweight="bold")
            ax.legend(framealpha=0)
...
# ─────────────────────────────────────────────
    ...
    try:
        ...
        for i, rot in enumerate(rotulos):
            ...
            m_path = os.path.join(pasta, "rf_modelo.joblib")
        ...
        if completos:
            ...
            with col1:
                ...
                ax.set_title("MSE por Grupo — Teste", fontweight="bold")
            ...
            with col2:
                ...
                for b, v in zip(bars2, [r["R²"] for r in completos]):
                    ax.text(b.get_x()+b.get_width()/2, max(v, 0)+0.01,
                            f"{v:.3f}", ha="center", va="bottom", fontsize=9, fontweight="bold")
                ax.set_title("R² por Grupo — Teste", fontweight="bold")
    ...
    with col_result:
        ...
        if prever:
            ...
            for nome, (mdir, mfile) in {
                "RF (Português)": (RF_PT_DIR, "rf_modelo_final.joblib"),
                "RF (English)":   (RF_EN_DIR, "rf_model_final.joblib"),
            }.items():
...
# ─────────────────────────────────────────────
    ...
    with col_conf:
        ...
        checks = {
            "RF PT":          os.path.join(RF_PT_DIR, "rf_modelo_final.joblib"),
            "RF EN":          os.path.join(RF_EN_DIR, "rf_model_final.joblib"),
            "RF Quintis D1":  os.path.join(QNT_DIR, "D1", "rf_modelo.joblib"),
            "MLP":            os.path.join(MLP_DIR, "modelo_cbr.keras"),
        }
