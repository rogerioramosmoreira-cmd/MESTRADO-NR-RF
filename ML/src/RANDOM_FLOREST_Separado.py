...
SEED            = 42
NUM_GRUPOS      = 5
TEST_SIZE       = 0.25    # 25% por grupo para teste
VAL_SIZE        = 0.20    # 20% do restante para validação
N_ITER_BUSCA    = 80      # Busca reduzida para grupos menores
CV_FOLDS        = 5       # 5-fold em grupos pequenos
...
OUTPUT_DIR = os.path.normpath(os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "models", "rf_quintis"
))
...
CORES_QUINTIL = {
    f"D{i+1}": color for i, color in enumerate([
        "#1D4ED8",
        "#0891B2",
        "#16A34A",
        "#D97706",
...
def encontrar_melhor_combinacao_features(X_treino, X_val, Y_treino, Y_val, feature_labels):
    """Avalia combinações de 1 ou 2 features e retorna as melhores pelo MSE."""
    combos = []
    inverse_target = np.expm1 if LOG_ALVO else lambda x: x
    Y_val_orig = inverse_target(Y_val)

    for tamanho in range(1, min(MAX_COMB_SIZE, X_treino.shape[1]) + 1):
        for cols in itertools.combinations(range(X_treino.shape[1]), tamanho):
            X_train_sub = X_treino[:, cols]
            X_val_sub = X_val[:, cols]
            scaler_sub = MinMaxScaler().fit(X_train_sub)
            X_train_sub = scaler_sub.transform(X_train_sub)
            X_val_sub = scaler_sub.transform(X_val_sub)
            ...
            modelo.fit(X_train_sub, Y_treino)
            preds = modelo.predict(X_val_sub)
            if LOG_ALVO:
                preds = np.expm1(preds)
            mse = mean_squared_error(Y_val_orig, preds)
            mae = mean_absolute_error(Y_val_orig, preds)
            combos.append({
                "cols": cols,
                "labels": [feature_labels[i] for i in cols],
                "mse": mse,
                "mae": mae,
                "r2": r2_score(Y_val_orig, preds),
            })

    combos.sort(key=lambda x: x["mse"])
    return combos[:TOP_N_COMB]
...
def treinar_quintil(rotulo, mask, X_full, Y_orig_full, Y_log_full):
    ...
    sw_treino = np.where(Y_treino > mediana_g, W_MINOR, W_MAJOR).astype(np.float32) if USE_WEIGHTS else None
    sw_tv = np.where(Y_tv > mediana_g, W_MINOR, W_MAJOR).astype(np.float32) if USE_WEIGHTS else None
    ...
    kf = KFold(n_splits=min(CV_FOLDS, max(2, len(X_treino_n) // 5)),
    ...
    melhores_combinacoes = encontrar_melhor_combinacao_features(
        X_treino, X_val, Y_treino, Y_val, FEATURES_LABELS
    )
    melhor_combo = melhores_combinacoes[0] if melhores_combinacoes else None
    if melhor_combo is not None:
        combo_text = ", ".join(melhor_combo["labels"])
        print(f"  Melhor combinação de features: {combo_text}  |  MSE Val={melhor_combo['mse']:.4f}")
    ...
    dump(rf_final, os.path.join(pasta_g, "rf_modelo.joblib"))
    ...
    return {
        ...
        "scaler":        scaler,
        "melhores_combinacoes": melhores_combinacoes,
        "melhor_combinacao": melhor_combo,
        "X_val":         X_val,
        "Y_val_orig":    Y_orig_val,
    }
...
def grafico_distribuicao_quintis(Y_orig, limites, rotulos):
    ...
    for i, rot in enumerate(rotulos):
        lo = limites[i]
        hi = limites[i + 1]
        cor = CORES_QUINTIL.get(rot, PALETTE["azul2"])
        ax.axvspan(lo, hi, alpha=0.12, color=cor, label=f"{rot} [{lo:.1f}–{hi:.1f}]")
        ax.axvline(lo, color=cor, lw=1.2, linestyle="--", alpha=0.7)

    ax.axvline(limites[-1], color=PALETTE["vermelho"], lw=1.2, linestyle="--", alpha=0.7)
    ax.set_title("Distribuição do CBR e Divisão em Quintis (D1–D5)", fontweight="bold")
...
def grafico_previsto_vs_real_grupo(res):
    rot = res["rotulo"]
    cor = CORES_QUINTIL.get(rot, PALETTE["azul"])

    fig, axes = plt.subplots(1, 2, figsize=(12, 5), facecolor=PALETTE["fundo"])
    fig.suptitle(f"{rot} — Previsto vs Real  "
                 f"(CBR {res['cbr_min']:.1f}–{res['cbr_max']:.1f}%)",
    ...
    for ax, (y_true, y_pred, met, conjunto) in zip(axes, dados):
        ...
        ax.set_title(f"{conjunto}  |  R²={met['r2']:.4f}  MSE={met['mse']:.4f}",
                     fontweight="bold")
...
def grafico_residuos_grupo(res):
    ...
    fig.suptitle(f"{rot} — Resíduos", fontsize=13, fontweight="bold")
    ...
    for ax, (y_true, y_pred, conjunto) in zip(axes, dados):
        ...
        ax.set_title(f"Resíduos — {conjunto}", fontweight="bold")
...
def grafico_busca_grupo(res):
    ...
    fig.suptitle(f"{rot} — Busca de Hiperparâmetros "
                 f"(CBR {res['cbr_min']:.1f}–{res['cbr_max']:.1f}%)",
                 fontsize=13, fontweight="bold")
    ...
    ax.set_title("Evolução da Busca — Marcos de Melhora", fontweight="bold")
    ax.legend(framealpha=0, fontsize=8)
    ...
    ax2.set_title("Top 10 Configurações  (barra = std CV)", fontweight="bold")
...
def grafico_importancia_grupo(res):
    ...
    ax.invert_yaxis()
    ax.set_title(f"{rot} — Importância das Features  "
                 f"(CBR {res['cbr_min']:.1f}–{res['cbr_max']:.1f}%)",
                 fontweight="bold")
...
def grafico_melhor_combinacao(res):
    rot = res["rotulo"]
    cor = CORES_QUINTIL.get(rot, PALETTE["azul"])
    combos = res["melhores_combinacoes"]
    best = res["melhor_combinacao"]

    fig, axes = plt.subplots(1, 2, figsize=(14, 5), facecolor=PALETTE["fundo"])
    fig.suptitle(f"{rot} — Melhores Combinações de Features",
                 fontsize=14, fontweight="bold")

    ax = axes[0]
    ax.set_facecolor(PALETTE["fundo"])
    ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
    labels = [", ".join(c["labels"]) for c in combos]
    mses = [c["mse"] for c in combos]
    ax.barh(range(len(labels)), mses, color=[cor if i == 0 else PALETTE["azul2"] for i in range(len(labels))], edgecolor="white")
    ax.set_yticks(range(len(labels)))
    ax.set_yticklabels(labels, fontsize=9)
    ax.invert_yaxis()
    ax.set_xlabel("MSE Validação")
    ax.set_title("Top combinações de 1 a 2 features", fontweight="bold")
    ...
    if best is not None:
        cols = best["cols"]
        X_val = res["X_val"][:, cols]
        y_true = res["Y_val_orig"]
        if len(cols) == 2:
            scatter = ax2.scatter(X_val[:, 0], X_val[:, 1], c=y_true, cmap="viridis",
                                   edgecolor="white", linewidths=0.4, s=55)
            ax2.set_xlabel(best["labels"][0])
            ax2.set_ylabel(best["labels"][1])
            ax2.set_title("Melhor par de features colorido por CBR", fontweight="bold")
            fig.colorbar(scatter, ax=ax2, label="CBR (%)")
        else:
            ax2.scatter(X_val[:, 0], y_true, alpha=0.75, color=cor, edgecolors="white", linewidths=0.4, s=60)
            ax2.set_xlabel(best["labels"][0])
            ax2.set_ylabel("CBR (%)")
            ax2.set_title("Melhor feature isolada", fontweight="bold")
    else:
        ax2.text(0.5, 0.5, "Nenhuma combinação disponível",
...
def grafico_comparativo_final(resultados):
    ...
    fig.suptitle("Comparativo Final — Random Forest por Faixa de CBR (D1–D5)",
    ...
    ax_mse.set_title("MSE por Grupo — Teste", fontweight="bold")
    ...
    for bar, val in zip(bars2, r2_vals):
        ax_r2.text(bar.get_x() + bar.get_width() / 2, max(val, 0) + 0.01,
                   f"{val:.3f}", ha="center", va="bottom", fontsize=9, fontweight="bold")
    ax_r2.set_title("R² por Grupo — Teste", fontweight="bold")
    ...
    for bar, val in zip(bars3, mae_vals):
        ax_mae.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.01,
                    f"{val:.3f}", ha="center", va="bottom", fontsize=9, fontweight="bold")
    ax_mae.set_title("MAE por Grupo — Teste", fontweight="bold")
    ...
    ax_scatter.set_title("Previsto vs Real — Todos os Grupos Sobrepostos",
                         fontweight="bold")
    ...
    ax_n.set_title("Nº de Amostras por Grupo", fontweight="bold")
    ax_n.set_ylabel("Amostras")

    ax_tab = fig.add_subplot(gs[2, :])
    ax_tab.axis("off")
    cab = ["Grupo", "Faixa CBR", "N", "Melhor MSE CV",
           "MSE Teste", "RMSE", "MAE", "R²", "Meta",
           "n_estimators", "max_depth", "Best combo"]
    linhas = []
    for res_g in resultados:
        mt = res_g["met_teste"]
        bp = res_g["best_params"]
        bc = res_g["melhor_combinacao"]
        combo_text = ", ".join(bc["labels"]) if bc is not None else "—"
    ...
    ax_tab.set_title("Resumo Comparativo — Métricas, Hiperparâmetros e Combinações de Features",
...
def grafico_importancia_comparativa(resultados):
    n_feat = len(FEATURES)
    matriz = np.zeros((len(resultados), n_feat))
    rotulos_g = []
    ...
    ax.set_title("Importância das Features por Faixa de CBR\n"
                 "(maior = mais relevante naquele grupo)", fontweight="bold")
    plt.tight_layout(); plt.show()


def fluxograma_arvore_aleatoria():
    fig, ax = plt.subplots(figsize=(14, 7), facecolor=PALETTE["fundo"])
    ax.axis("off")

    caixas = [
        (0.05, 0.70, "Carregar dados"),
        (0.35, 0.70, "Calcular quintis"),
        (0.65, 0.70, "Treinar RF por grupo"),
        (0.35, 0.40, "Validar e escolher\nmelhor combinação"),
        (0.65, 0.40, "Avaliar métricas\npor grupo"),
        (0.35, 0.10, "Comparar resultados\ne salvar modelos"),
    ]

    for x, y, texto in caixas:
        patch = FancyBboxPatch((x, y), 0.25, 0.16,
                               boxstyle="round,pad=0.04",
                               linewidth=1.8, edgecolor=PALETTE["azul"],
                               facecolor="#FFFFFF")
        ax.add_patch(patch)
        ax.text(x + 0.125, y + 0.085, texto, ha="center", va="center",
                fontsize=11, fontweight="bold")

    conexoes = [
        ((0.30, 0.78), (0.35, 0.78)),
        ((0.60, 0.78), (0.65, 0.78)),
        ((0.475, 0.70), (0.475, 0.56)),
        ((0.50, 0.56), (0.50, 0.50)),
        ((0.50, 0.34), (0.50, 0.26)),
        ((0.60, 0.50), (0.65, 0.50)),
    ]
    for p0, p1 in conexoes:
        ax.annotate("", xy=p1, xytext=p0,
                    arrowprops=dict(arrowstyle="->", color="#1F2937", lw=2))

    ax.set_title("Fluxograma do pensamento da Árvore Aleatória",
...
Y_orig = df[COLUNA_ALVO].values.ravel()
...
rotulos_quintis = [f"D{i+1}" for i in range(NUM_GRUPOS)]
mascaras = []
for i in range(NUM_GRUPOS):
    ...
    if n > 0:
        ...
        print(f"  {rotulos_quintis[i]}: {n:3d} amostras  (vazio)")
...
for res_g in resultados:
    mt = res_g["met_teste"]
    combo_text = ", ".join(res_g["melhor_combinacao"]["labels"]) if res_g["melhor_combinacao"] else "—"
